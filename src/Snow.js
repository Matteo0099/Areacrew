...
function updatePositions() {
    flakes.forEach((flake) => {
        ...
        if (dt <= 1.0) {
            ...
            flake.style.transform = `translate3d(${x}vw, ${y}vh, ${z}px)`;
            if (dt >= 0.5) { //Start fading out flakes 1/2 down screen
                flake.style.opacity = 1.0 - (dt - 0.5) * 2;
            }
            curLife -= LIFE_PER_TICK;
            flake.dataset.life = curLife;
        } else { //Once the lifespan is exceeded, reset the flake
            resetFlake(flake);
        }
    });
...
function appendSnow() {
    ...
    const addFlake = () => { //This makes it much less clumpy
        ...
        if (i++ <= MAX_FLAKES) { //Recursive (delayed by timeout) call to add a flake until max reached
